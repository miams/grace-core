...
resource "aws_cloudformation_stack" "staging_waf" {
  ...
  parameters {
    AccessLogBucket                        = "${var.staging_access_log_bucket}"
    ...
    "Resources": {
      ...
      "LambdaWAFLogParserFunction": {
        ...
        "Properties": {
          "Description": {
            "Fn::Join": ["", [
              "This function parses  access logs to identify suspicious behavior, such as an abnormal amount of requests or errors. It then blocks those IP addresses for a customer-defined period of time. Parameters: ", {
